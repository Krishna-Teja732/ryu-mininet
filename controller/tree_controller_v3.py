...
print(
    f"Resource Limits for open files(soft, hard): {resource.getrlimit(resource.RLIMIT_NOFILE)}"
)
...
class TreeControllerV3(OSKenApp):
    ...
    def __init_flood_flow_rules(self, datapath, ofp, ofp_parser):
        ...
        if not is_core_switch:
            ...
            partial_flood_actions.append(
                ofp_parser.OFPActionOutput(ofp.OFPP_CONTROLLER)
            )
            for in_port in range(1, (self.branch_factor // 2) + 1):
                match = ofp_parser.OFPMatch(
                    in_port=in_port, eth_dst="FF:FF:FF:FF:FF:FF"
                )
    ...
    def store_port_desc_stats(self, ev):
        ...
        for port in ev.msg.body:
            ...
            if (
                port.port_no == (ofp.OFPP_CONTROLLER + 1)
                or port.port_no == ofp.OFPP_CONTROLLER
            ):
                continue

            if port.port_no not in self.port_num_to_obj_map[datapath.id]:
                self.port_num_to_obj_map[datapath.id][port.port_no] = SwitchPort(
                    port.hw_addr, port.port_no, datapath.id
                )

            lldp_packet = LLDPPacket.lldp_packet(
                datapath.id, port.port_no, port.hw_addr, 0
            )
    ...
    def flow_removed_handler(self, ev):
        # TODO: Store the action for each flow match, send both flow match and flow action to kgbuilder
        msg = ev.msg
    ...
    def packet_in_handler(self, ev):
        ...
        if eth_headers.ethertype == ether_types.ETH_TYPE_LLDP:
            ...
            self._link_add_handler(
                src_dpid, src_port, datapath.id, switch_in_port.number
            )
        ...
        if switch_in_port not in forward_table[eth_src]:
            ...
            self.logger.debug(
                f"{eth_src} reachable through switch {datapath.id} port {switch_in_port}"
            )

        # Check port number range. For k = 12, even in peer type is switch, the mac is learnt
        # leading to incorrect topology
        if (
            switch_in_port.peer_type == PeerType.HOST
            and switch_in_port.number > self.branch_factor // 2
            and eth_src not in self.host_mac_set
        ):
        ...
        if eth_dst not in forward_table or len(forward_table[eth_dst]) == 0:
            self.logger.warning(
                f"WARN: {eth_dst} not reachable through switch {datapath.id}"
            )
            return

        flow = f"{switch_in_port.number}:{eth_dst}"
        if flow not in self.dpid_flows[datapath.id]:
            ...
            match = ofp_parser.OFPMatch(in_port=switch_in_port.number, eth_dst=eth_dst)
